...
namespace Ecms.Biz
{
    public class OrderBiz : Respository
    {
		public OrderBiz()
			: base()
		{
                ...
                , ref string alParamsOutError)
        {
			...
			{
				#region // Query:
				var query = from o in this.EcmsDataContext.Orders
							join c in this.EcmsDataContext.Customers on o.CustomerId equals c.CustomerId into c_join
							from c in c_join.DefaultIfEmpty()
							join u in this.EcmsDataContext.Sys_User on c.UserCode equals u.UserCode
							join ep in this.EcmsDataContext.Sys_User on o.EmployeeCode equals ep.UserCode into epc
							from ep in epc.DefaultIfEmpty()
							where c.Status == 0 //&& o.OrderId == 486
							select new OrderModel()
							{
								...
								ConfigRateValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == o.ConfigRateId).ConfigValue),

								lstOrderDetailModel = (from p in this.EcmsDataContext.OrderDetails
													   join pr in this.EcmsDataContext.Products on (p.ProductId ?? 0) equals pr.ProductId into pr_join
													   from pr in pr_join.DefaultIfEmpty()
													   join ci in this.EcmsDataContext.Countries on p.CountryId equals ci.CountryId into ci_join
													   from ci in ci_join.DefaultIfEmpty()
													   join cc in this.EcmsDataContext.Categories on p.CategoryId equals cc.CategoryId into cc_join
													   from cc in cc_join.DefaultIfEmpty()
													   join pc in this.EcmsDataContext.Categories on cc.ParentId equals pc.CategoryId into pc_join
													   ...
													   select new OrderDetailModel
													   {
														   ...
														   EffortConfigValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.EffortConfigId).ConfigValue),
														   ...
														   ShipConfigValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.ShipConfigId).ConfigValue),
														   ...
														   TaxUsaConfigValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.TaxUsaConfigId).ConfigValue),

														   InsuaranceConfigId = p.InsuaranceConfigId,
														   InsuaranceConfigValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.InsuaranceConfigId).ConfigValue),
														   ...
														   RateCountryValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.RateCountryId).ConfigValue ?? 0),
														   RateUSDValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == p.Order.ConfigRateId).ConfigValue ?? 0),
														   OrderStatus = o.OrderStatus,
                                                           TaxUsaModified=p.TaxUsaModified,
													   }
							)
							};
            ...
            try
            {
                using (var ts = new System.Transactions.TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
                        ...
                        #region // Build:
                        // delete customer:
                        dbContext.Orders.DeleteObject(delOrder);
            ...
            , ref string alParamsOutError)
        {
            ...
            try
            {
                using (var tranScope = new TransactionScope())
                {
                    ...
                    dbContext.OrderHistories.AddObject(orderHistory);
            ...
            try
            {
                ...
                using (var db = new EcmsEntities())
                {
                    ...
                    var details = from p in db.OrderDetails
                                  ...
                                  from pr in pr_join.DefaultIfEmpty()
                                  join w in db.WebsiteLinks on (p.WebsiteId ?? 0) equals w.WebsiteId into w_join
                    ...
                    #region // return:
                    return details.OrderBy(x => x.WebsiteId).ToList();
                    #endregion
                }
            }
            catch (Exception ex)
                        ...
                        if (customer != null)
                        {
							...
							if (invoices != null && invoices.Count() > 0)
							{
								...
								if (oAmount > 0)
								{
									...
									dbContext.CustomerBalanceHistories.AddObject(customerBalanceHistory);
									...
									dbContext.Invoices.AddObject(ivoi);
								}
							}
                        }
			...
			, ref string alParamsOutError)
		{
			...
			try
			{
				using (var tranScope = new TransactionScope())
				{
					...
					orderReturn.Description = string.Format("Hệ thống đã Hoàn lại tình trạng chưa xác nhận cho đơn hàng!");
		...
		#region // RevertFirstOrderStatus01
		public bool RevertFirstOrderStatus01(
			string orderId
			, ref string alParamsOutError)
		{
			#region // Temp
			string strFunctionName = "RevertFirstOrderStatus01";
			var alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName
                        , "orderId",orderId
			            });
			...
			try
			{
				using (var ts = new TransactionScope())
				{
					#region // Check:
					int oId = Convert.ToInt32(orderId);
					var orderReturn = this.EcmsDataContext.Orders.SingleOrDefault(x => x.OrderId == oId);
					...
					// Chỉ cho revert ở 2 tình trạng là OrderConfirmed + Finished
					if (orderReturn.OrderStatus != OrderInStatus.OrderConfirmed && orderReturn.OrderStatus != OrderInStatus.Finished && orderReturn.OrderStatus != OrderInStatus.Deliveried)
					{
						throw new Exception("Đơn hàng không ở tình trạng Confirm hoặc Finish hoặc Deliveried");
					}

					var customer = this.EcmsDataContext.Customers.Where(x => x.CustomerId == orderReturn.CustomerId).SingleOrDefault();
					if (customer == null)
					{
						throw new Exception("Thông tin KH không đúng");
					}
					...
					// Check created invoice for Order
					var invoices = from p in this.EcmsDataContext.InvoiceDetails
								   where (p.Invoice.OrderId == oId
											&& p.Invoice.CustomerId == customer.CustomerId
											&& ((p.Invoice.BusinessCode == Const_BusinessCode.Business_208 || p.Invoice.BusinessCode == Const_BusinessCode.Business_209) && p.Invoice.Status == InvoiceStatus.Confirm)
											)
								   select p;
					...
					if (invoices != null && invoices.Count() > 0)
					{
						// throw new Exception("Đơn hàng đã được lập phiếu thanh toán, bạn không thể hoàn lại tình trạng");
						// Hủy thanh toán
						oAmount = invoices.Sum(p => p.Amount ?? 0);
						if (oAmount > 0)
						{
							// 1. hủy các thanh toán kèm theo
							foreach (var item in invoices)
							{
								item.Invoice.Status = InvoiceStatus.NotConfirm;
								item.Invoice.Description = string.Format("Hủy thanh toán do MBGN hoàn lại trạng thái chưa xác nhận cho đơn hành");
								item.Invoice.LastDateModify = DateTime.Now;
							}
						}
					}

					#region // phân bổ lại nếu có thanh toán
					if (oAmount > 0)
					{
						// 2. Phân bổ thanh toán cho đơn hàng khác
						NLogLogger.Info(string.Format("RevertFirstOrderStatus01, bat dau phan bo lai thanh toan"));
						// lấy danh sách order
						var orderList = from p in this.EcmsDataContext.Orders
										where p.CustomerId == customer.CustomerId && (p.OrderStatus == OrderInStatus.OrderConfirmed
						...
						//{
						//    #region // Tính tổng tiền đơn hàng
						//    var lstOrderDetail = this.EcmsDataContext.OrderDetails.Where(p => p.OrderId == item.OrderId);
						//    double sumOrderAmount = 0;
						//    foreach (var itemDetail in lstOrderDetail)
						//    {
						//        sumOrderAmount += this.GetDetailTotalMoney(itemDetail.OrderDetailId);
						//    }
						...
						//    NLogLogger.Info(string.Format("sumAmount of Order:{0}, sumOrderAmount={1}", item.OrderNo, sumOrderAmount));

						//    // Tính số tiền đã thanh toán
						//    var paidOrderAmount = Math.Round((this.GetTotalPayAmountNormal(item.OrderId)), Constansts.NumberRoundDefault);

						//    NLogLogger.Info(string.Format("amount paid for Order:{0}, paidOrderAmount={1}", item.OrderNo, paidOrderAmount));

						//    var remainAmount = (double)(sumOrderAmount - paidOrderAmount);
						...
						//        continue;
						//    }
						//    var newAmount = (remainAmount >= oAmount) ? oAmount : remainAmount;

						//    if (newAmount > 0)
						//    {
						//        //Convert.ToInt16("123s");
						//        NLogLogger.Info(string.Format("RevertFirstOrderStatus01, start for fuc InvoiceConfirmPayment01 oAmount={0}, newAmount={1}", oAmount, newAmount));

						//        //var newResultPayment = new InvoiceBiz().InvoiceConfirmPayment01(
						//        //                        Convert.ToString(customer.CustomerId)
						//        //                        , ""
						//        //                        , ""
						//        //                        , Convert.ToString(item.OrderId)
						//        //                        , Convert.ToString(newAmount)
						//        //                        , string.Format("Hệ thống phân bổ thanh toán cho đơn hàng khi hoàn lại TT")
						//        //                        , Const_BusinessCode.Business_209
						//        //                        , ""
						//        //                        , DateTime.Now.ToString("yyyy-MM-dd")
						//        //                        , ref alParamsOutError
						//        //                        );

						//        var newResultPayment = this.InvoiceConfirmPayment011(
						//                                Convert.ToString(customer.CustomerId)
						//                                , ""
						//                                , ""
						//                                , Convert.ToString(item.OrderId)
						...
						//    if (newAmount <= 0 || oAmount <= 0)
						//    {
						//        NLogLogger.Info(string.Format("RevertFirstOrderStatus01, het tien phan bo lai, newAmount:{0}, oAmount:{1}", newAmount, oAmount));
						//        break;
						//    }
						//}
						#endregion

						// Nếu đã giao hàng => Hoàn lại số dư cho tài khoản KH
						if (orderReturn.OrderStatus == OrderInStatus.Deliveried)
						{
							NLogLogger.Info(string.Format("RevertFirstOrderStatus01, start for fuc Order revert status = Deliveried"));
							// Lấy số tiền đã trừ:
							var invoiceDeliveried = from p in this.EcmsDataContext.InvoiceDetails
													where (p.Invoice.OrderId == oId
															 && p.Invoice.CustomerId == customer.CustomerId
															 && (p.Invoice.BusinessCode == Const_BusinessCode.Business_206 && p.Invoice.Status == InvoiceStatus.Confirm)
															 )
							...
							if (invoices != null && invoices.Count() > 0)
							{
								var oAmountDeliveried = invoices.Sum(p => p.Amount ?? 0);
								if (oAmountDeliveried > 0)
								{
									// Hoàn lại số dư cho KH
									customer.Balance += oAmountDeliveried;
									...
									customerBalanceHistory.CustomerId = customer.CustomerId;
									customerBalanceHistory.CreatedDate = DateTime.Now;
									EcmsDataContext.CustomerBalanceHistories.AddObject(customerBalanceHistory);

									foreach (var Deliveried in invoiceDeliveried)
									{
										Deliveried.Invoice.Status = InvoiceStatus.NotConfirm;
										Deliveried.Invoice.Description = string.Format("Hủy thanh toán do MBGN hoàn lại trạng thái Hoàn thành cho đơn hành");
										Deliveried.Invoice.LastDateModify = DateTime.Now;
									}
								}
							}
						}
					...
					// 3. Revert status to OrderPending
					orderReturn.OrderStatus = OrderInStatus.OrderPending;
					orderReturn.ConfirmDate = null;
					orderReturn.Description = string.Format("Hệ thống đã Hoàn lại tình trạng chưa xác nhận cho đơn hàng!");
					...
					orderHistory.CreatedDate = DateTime.Now;
					orderHistory.Remark = string.Format("revert to Pending(3)");
					this.EcmsDataContext.OrderHistories.AddObject(orderHistory);

					#region // Nếu đã tính phí trả chậm thì phải tính toán lại
					if (orderReturn.CalFeeDelay == Constansts.CalFeeDelay
						&& orderReturn.IsCalFeeDelay == Constansts.FlagActive
						&& orderReturn.OrderTypeId == OrderType_Const.OrderBylink
						)
		...
		#region // clone from InvoiceBiz because transaction not work from invoiceBiz
		// Phân bổ cho đơn hàng từ số dư khả dụng: businesscode =209
		public bool InvoiceConfirmPayment011(
				string customerId
				, string bankId
				, string fromAccount
		...
		{
			#region // Temp
			string strFunctionName = "InvoiceConfirmPayment011";
			var alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName,
						"customerId", customerId
						, "bankId", bankId
			...
			try
			{
				...
				// Check Mã hóa đơn:
				if (this.EcmsDataContext.Invoices.Any(x => x.InvoiceCode == invoice.InvoiceCode))
				{
					invoice.InvoiceCode = "I" + DateTime.Now.AddSeconds(01).ToString("yyMMddHHmmss");
				...
				this.EcmsDataContext.Invoices.AddObject(invoice);
				this.EcmsDataContext.SaveChanges();
		...
		public bool CheckPaymentForOrder(int orderId)
		{
			return (this.EcmsDataContext.Invoices.Any(p => p.OrderId == orderId
													&& ((p.BusinessCode == Const_BusinessCode.Business_208 || p.BusinessCode == Const_BusinessCode.Business_209)
													&& p.Status == InvoiceStatus.Confirm)
													));
		}

		#region // RevertOrderPendingByDetailCode
		/// <summary>
		/// Revert money for customer account when cancel DetailCode
		/// </summary>
		/// <param name="orderId"></param>
		/// <param name="alParamsOutError"></param>
		/// <returns></returns>
		public bool RevertOrderPendingByDetailCode(
			List<int> lstDetailId //string orderId
			, ref string alParamsOutError)
		{
			#region // Temp
			string strFunctionName = "RevertOrderPendingByDetailCode";
			var alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName
			            });
			...
			try
			{
				...
				//{
					...
					foreach (var detailId in lstDetailId)
					{
						var orderDetail = this.EcmsDataContext.OrderDetails.SingleOrDefault(p => p.OrderDetailId == detailId);
						if (orderDetail == null)
						{
							NLogLogger.Info(string.Format("RevertOrderPendingByDetailCode, cannot find OrderdetailId detailId: {0}", detailId));
							continue;
						}

						// 1. Check order is exists payment?
						if (!this.CheckPaymentForOrder(orderDetail.OrderId))
						{
							NLogLogger.Info(string.Format("RevertOrderPendingByDetailCode, payment for order does not exists, detailId: {0}, orderId: {1}", detailId, orderDetail.OrderId));
							continue;
						}

						//// ==> if exists payment
						//var orderByDetail = this.OrderGet(
						//        Convert.ToString(orderDetail.OrderId)
						//        , ""
						//        , ""
						//        , ""
						//        , ""
						...
						//        , ""
						//        , ""
						//        , ref alParamsOutError).SingleOrDefault();
						 ...
						 * - paymentAmount
						*/
						// 2. if order is exists payment then check payment number?
						// 2.1 if TotalMoney <= remainAmount
						var totalRemailAmount = this.GetTotalMoney(orderDetail.OrderId)  - this.GetTotalPayAmountNormal(orderDetail.OrderId);
						var totalDetailMoney = this.GetDetailTotalMoney(orderDetail.OrderDetailId);

						//var orderDetailModel = orderByDetail.lstOrderDetailModel.ToList().Where(p => p.OrderDetailId == detailId).SingleOrDefault();
						//if (orderDetailModel.TotalMoney <= orderByDetail.RemainAmount)
						if (totalDetailMoney <= totalRemailAmount)
						{
							NLogLogger.Info(string.Format("RevertOrderPendingByDetailCode, orderDetailModel.TotalMoney <= orderByDetail.RemainAmount, totalDetailMoney: {0}, RemainAmount: {1}", totalDetailMoney, totalRemailAmount));
							continue;
						}
						else
						{
							NLogLogger.Info(string.Format("RevertOrderPendingByDetailCode, orderDetailModel.TotalMoney > orderByDetail.RemainAmount, totalDetailMoney: {0}, RemainAmount: {1}", totalDetailMoney, totalRemailAmount));
							throw new Exception("TotalMoney > RemainAmount of Orderdetail, cannot revert payment");
						}

						// 2.1 if TotalMoney > remainAmount
						// revert all payment for this order

						//NLogLogger.Info(string.Format("RevertOrderPendingByDetailCode, start call RevertFirstOrderStatus01"));
						//this.RevertFirstOrderStatus01(Convert.ToString(orderDetail.OrderId), ref alParamsOutError);
					}
					#endregion
				...
				, ref string alParamsOutError)
		{
			...
			try
			{
				...
				int oDetailId = Convert.ToInt32(orderDetailId);
				var orderDetailReturn = this.EcmsDataContext.OrderDetails.SingleOrDefault(x => x.OrderDetailId == oDetailId);
				...
				if (!string.IsNullOrEmpty(orderDetailStatusNew) & OrderOutboundStatus.Cancel == Convert.ToInt32(orderDetailStatusNew))
				{
					// Check hoàn lại thanh toán
					var lstDetailId = new List<int>() { oDetailId };
					this.RevertOrderPendingByDetailCode(lstDetailId, ref alParamsOutError);
				}
		...
		public OrderDetail OrderDetailUpdate(OrderDetail orderDetail, ref string alParamsOutError)
        {
            #region // Temp
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
					...
					orderDetailReturn.RateCountryId = orderDetail.RateCountryId;
                    if (orderDetail.PriceWeb != null)
                    {
                        orderDetailReturn.PriceWeb = orderDetail.PriceWeb;
                    }
                    
                    orderDetailReturn.TaxUsaModified = orderDetail.TaxUsaModified;
                    dbContext.SaveChanges();
                    #endregion
        ...
        public bool OrderDetailDelete(string orderDetailId, ref string alParamsOutError)
        {
            #region // Temp
            string strFunctionName = "OrderDetailDelete";
            var alParamsCoupleError = new ArrayList(new object[]{
                        "strFunctionName", strFunctionName
						, "orderDetailId", orderDetailId
                        });
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    if (OrderInStatus.OrderPending == (orderDetailReturn.Order.OrderStatus??0))
                    {
                        if((OrderOutboundStatus.Cancel == (orderDetailReturn.DetailStatus??0))
                            || (OrderOutboundStatus.NoStatus == (orderDetailReturn.DetailStatus ?? 0))
                            || (OrderOutboundStatus.InProcess == (orderDetailReturn.DetailStatus ?? 0)))
                        {
                            dbContext.OrderDetails.DeleteObject(orderDetailReturn);
                            dbContext.SaveChanges();
                        }
                    }
                ...
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    foreach (var item in listOrderDetailModel)
                    {
                        ...
                        if (orderDetailReturn == null)
                        {
                            ...
                            dbContext.OrderDetails.AddObject(orderDetail);
                        }
                    }
                    dbContext.SaveChanges();
		...
		public OrderDetail OrderDetailDeliveryUpdate(OrderDetail orderDetail, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
					...
					orderDetailReturn.LotPrice = orderDetail.LotPrice;
		...
		#region // GetTotalMoney
		public double GetTotalMoney(int orderId)
		{
			var orderDetails = this.EcmsDataContext.OrderDetails.Where(p => p.OrderId == orderId);
			double totalMoney = 0.0;
			foreach (var item in orderDetails)
			{
				totalMoney += this.GetDetailTotalMoney(item.OrderDetailId);
			}
			return totalMoney;
		}
		#endregion
		...
		{
			// Lấy Detail
			var detail = this.EcmsDataContext.OrderDetails.SingleOrDefault(p => p.OrderDetailId == orderDetailId);
			...
			var configBs = this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == detail.EffortConfigId);
			double effortConfigValue = configBs == null ? 0 : configBs.ConfigValue ?? 0;

			// tính ShipConfigValue
			var configShip = this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == detail.ShipConfigId);
			double shipConfigValue = configShip == null ? 0 : configShip.ConfigValue ?? 0;

			// tính InsuaranceConfigValue
			var configIns = this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == detail.InsuaranceConfigId);
			double insuaranceConfigValue = configIns == null ? 0 : configIns.ConfigValue ?? 0;

			// tính TaxUsaConfigValue
			var configTax = this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == detail.TaxUsaConfigId);
			...
			switch (detail.Order.OrderTypeId)
			{
				...
				case 2: //Đặt hàng Link
					...
					rateValue = detail.RateCountryId == null ? 0 : (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == (detail.RateCountryId)).ConfigValue ?? 0);
					

					var fee = ((detail.Weight ?? 0) * (detail.ShipModified != null ? detail.ShipModified.Value : shipConfigValue));
                    var tax = ((detail.TaxUsaModified != null ? detail.TaxUsaModified.Value : taxUsaConfigValue));
                    return (moneyProduct + fee + (moneyProduct * (detail.EffortModified == null ? effortConfigValue : (detail.EffortModified ?? 0)) * 0.01) + (moneyProduct * tax * 0.01) + (detail.Surcharge ?? 0) + (detail.ShipUSA ?? 0)) * rateValue;

				case 3: //Vận chuyển
					//Lấy tỷ giá USD
					rateValue = (this.EcmsDataContext.ConfigBusinesses.FirstOrDefault(x => x.ConfigBusinessId == detail.Order.ConfigRateId).ConfigValue ?? 0);
		...
		{
			var money = (from id in this.EcmsDataContext.InvoiceDetails
						 join i in this.EcmsDataContext.Invoices on id.InvoiceId equals i.InvoiceId
			...
			try
			{
				...
				using (var dbContext = new EcmsEntities())
				{
					var query = from p in dbContext.OrderDetails
								...
								from ci in ci_join.DefaultIfEmpty()
								where p.Order.Customer.Status == 0 && (p.Order.OrderTypeId == 2 || p.Order.OrderTypeId == 3) && p.Order.OrderStatus != OrderInStatus.OrderCancel
                    ...
                    {
                        int id = Convert.ToInt32(detailId);
                        query = query.Where(p => p.OrderDetailId == id);
                    }
            ...
            try
            {
                ...
                using (var db = new EcmsEntities())
                {
                    ...
                    var query = from o in db.OrderOutbounds
                                ...
                                from w in w_join.DefaultIfEmpty()
								//join ws in db.Mst_VisaAccount on o.vi equals ws.WebsiteAccountVisaId into ws_join
            ...
            try
            {
                using (var tranScope = new TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
                        ...
                        dbContext.AddToOrderOutbounds(orderOutbound);
        ...
        public OrderOutbound OrderOutboundUpdate(OrderOutbound orderOutbound, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var tranScope = new TransactionScope())
                {
                    ...
                    {
                        dbContext.OrderOutboundDetails.DeleteObject(item);
                    }
            ...
            , ref string alParamsOutError)
        {
            ...
            try
            {
                ...
                using (var tranScope = new TransactionScope())
                {
					...
					var oobReturn = this.EcmsDataContext.OrderOutbounds.SingleOrDefault(x => x.OrderOutboundId == outId);
                    if (oobReturn == null)
                    {
                        throw new Exception("Đơn hàng không tồn tại");
                    }

					var orderDetais = from od in this.EcmsDataContext.OrderDetails
									  join ood in this.EcmsDataContext.OrderOutboundDetails on od.OrderDetailId equals ood.OrderDetailId
					...
					foreach (var item in orderDetais)
					{
						...
						{
							//// check for revert
							//var totalRemailAmount = this.GetTotalMoney(item.OrderId) - this.GetTotalPayAmountNormal(item.OrderId);
							//var totalDetailMoney = this.GetDetailTotalMoney(item.OrderDetailId);
					...
					if (!string.IsNullOrEmpty(status) && OrderOutboundStatus.Cancel == Convert.ToInt32(status) && isCalFeeDelayAgain)
					{
						...
						var sysAuto = this.EcmsDataContext.Mst_SysAuto.Where(p => p.CreatedDate == currentDate).OrderByDescending(p => p.CreatedDate).FirstOrDefault();
						if (sysAuto != null)
						{
							NLogLogger.Info(string.Format("Delete sysAuto createDate={0}", currentDate.ToString("yyyy-MM-dd")));
							this.EcmsDataContext.Mst_SysAuto.DeleteObject(sysAuto);
							//dbContext.SaveChanges();
						}
					...
					this.EcmsDataContext.OrderOutboundHistories.AddObject(onHistory);
            ...
            try
            {
                using (var ts = new System.Transactions.TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
						...
						// delete customer:
						dbContext.OrderOutbounds. DeleteObject(delOrderOutbound);
            ...
            try
            {
                using (var tranScope = new TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
                        ...
                        dbContext.OrderOutboundDetails.DeleteObject(orderReturn);
		...
		public bool CalDelayFeeOrder(ref string alParamsOutError)
		{
			...
			try
			{
				...
				var configBusiness = this.EcmsDataContext.ConfigBusinesses.Where(x => x.BusinessCode == Const_BusinessCode.Business_301).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
				...
				var autoSys = this.EcmsDataContext.Mst_SysAuto.OrderByDescending(p => p.CreatedDate).FirstOrDefault();
				...
				#region // Query:
				var configBusinessDayAloowed = this.EcmsDataContext.ConfigBusinesses.Where(x => x.BusinessCode == Const_BusinessCode.Business_302).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
				...
				var configBusinessAddValue = this.EcmsDataContext.ConfigBusinesses.Where(x => x.BusinessCode == Const_BusinessCode.Business_303).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
				...
				foreach (var item in query)
				{
					...
					var order = this.EcmsDataContext.Orders.SingleOrDefault(p => p.OrderId == item.OrderId);
				...
				this.EcmsDataContext.Mst_SysAuto.AddObject(autoSysAdd);
						...
						using (var dbContext = new EcmsEntities())
						{
							...
							if (order != null && order.OrderStatus == OrderInStatus.Deliveried)
							{
								...
								foreach (var item in lstInvoice206)
								{
									...
									//dbContext.Invoices.DeleteObject(item);
							...
							{
								count++;
								dbContext.Invoices.AddObject(item);
								dbContext.SaveChanges();
    ...
    public class OrderDetailModel
    {
        ...
        public double? TaxUsaModified { get; set; }
		...
		//public double RateValue { set; get; }
			...
			get
			{
				switch (OrderTypeId)
				{
					...
					case 2:
                        ...
                        var tax = ((TaxUsaModified != null ? TaxUsaModified.Value : TaxUsaConfigValue));
                        return (MoneyProduct + (MoneyProduct * (EffortModified == null ? (EffortConfigValue ?? 0) : (EffortModified ?? 0)) * 0.01) + (MoneyProduct * (tax ?? 0) * 0.01) + (Surcharge ?? 0) + (ShipUSA ?? 0)) * RateCountryValue;
			...
			get
			{
				switch (OrderTypeId)
				{
					...
					case 2: //Đặt hàng Link
						...
						if (DetailStatus == 3) return 0;
                        var tax = ((TaxUsaModified != null ? TaxUsaModified.Value : TaxUsaConfigValue));
                        return FeeShip + (MoneyProduct + (MoneyProduct * (EffortModified == null ? (EffortConfigValue ?? 0) : (EffortModified ?? 0)) * 0.01) + (MoneyProduct * (tax ?? 0) * 0.01) + (Surcharge ?? 0) + (ShipUSA ?? 0)) * RateCountryValue;
        ...
        public string DetailStatusText
        {
            get
            {
                return DetailStatus == 1 ? OrderOutboundStatus.InProcessText :
                        ...
                        (OrderTypeId==2 && (DetailStatus == 0 || DetailStatus == null))?"Thay đổi":"";
            }
            set
            {
