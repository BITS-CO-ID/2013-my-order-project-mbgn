...
namespace Ecms.Biz
{
    ...
    public class CustomerBiz : ICustomerBiz
    {
            ...
            , ref string alParamsOutError)
        {
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    dbContext.Sys_User.AddObject(sysUser);
            ...
            try
            {
                using (var ts = new System.Transactions.TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
                        ...
                        #region // Build:
                        // delete customer:
                        dbContext.Customers.DeleteObject(delCustomer);

                        // delete user:
                        dbContext.Sys_User.DeleteObject(delUser);
                        #endregion
        ...
        #region // CustomerTemp
        public CustomerTemp CustomerTempCreate(CustomerTemp customerTemp, ref string alParamsOutError)
        {
            #region // Temp
            string strFunctionName = "CustomerTempCreate";
            ArrayList alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName
			            });
            #endregion
            try
            {
                using (var _db = new EcmsEntities())
                {
                    _db.CustomerTemps.AddObject(customerTemp);
                    _db.SaveChanges();
                    return customerTemp;
                }
            }
			...
			try
			{
				using (var _db = new EcmsEntities())
				{
					var result = from p in _db.ConfigBusinesses
								 join ct in _db.CustomerTypes on p.CustomerTypeId equals ct.CustomerTypeId into ct_join
								 from ct in ct_join.DefaultIfEmpty()
								 join ot in _db.OrderTypes on p.OrderTypeId equals ot.OrderTypeId into ot_join
								 from ot in ot_join.DefaultIfEmpty()
								 join t in _db.Countries on p.CountryId equals t.CountryId into t_join
								 from t in t_join.DefaultIfEmpty()
								 join c in _db.Categories on p.CategoryId equals c.CategoryId into c_join
								 from c in c_join.DefaultIfEmpty()
								 join b in _db.Mst_Business on p.BusinessCode equals b.BusinessCode into b_join
		...
		public ConfigBusiness ConfigBusinessCreate(ConfigBusiness ctcb, ref string alParamsOutError)
		{
			...
			try
			{
				using (var _db = new EcmsEntities())
				{
					_db.ConfigBusinesses.AddObject(ctcb);
					_db.SaveChanges();
					return ctcb;
				}
			}
		...
		public bool ConfigBusinessValueUpdate(
				string configId
				, string configValue // lock=1; unlock=0
				, ref string alParamsOutError)
		{
			#region // Temp
