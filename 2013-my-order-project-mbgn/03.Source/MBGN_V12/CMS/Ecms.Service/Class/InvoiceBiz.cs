...
namespace Ecms.Biz
{
    public class InvoiceBiz : Respository
    {
		public InvoiceBiz()
			: base()
		{
                ...
                , ref string alParamsOutError)
        {
            ...
            try
            {
                ...
                using (var db = new EcmsEntities())
                {
					...
					if (!string.IsNullOrEmpty(invoiceRefCode))
					{
						query = query.Where(x => x.InvoiceRefCode == invoiceRefCode);
					}
        ...
        public Invoice InvoiceCreate(Invoice invoice, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    dbContext.Invoices.AddObject(invoice);
		...
		#region // InvoiceConfirmPayment: businessCode = 201
        ...
        #region // InvoiceReturnPayment
        public Invoice InvoiceReturnPayment(Invoice invoice, ref string alParamsOutError)
        {
            #region // Temp
            string strFunctionName = "InvoiceReturnPayment";
            var alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName                      
			            });
            #endregion
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    #region // Xử lý hoàn tiền

                    var customer = dbContext.Customers.SingleOrDefault(x => x.CustomerId == invoice.CustomerId);
                    if (customer == null)
                    {
                        throw new Exception("Tài khoản khách hàng không tồn tại");
                    }
        ...
        public Invoice InvoiceUpdate(Invoice invoice, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var tranScope = new TransactionScope())
                {
                    ...
                    {
                        dbContext.InvoiceDetails.DeleteObject(item);
                    }

                    //Add lại Invoice detail mới
                    foreach (var od in invoice.InvoiceDetails)
                    {
                        dbContext.InvoiceDetails.AddObject(od);
                    }
            ...
            try
            {
                using (var ts = new System.Transactions.TransactionScope())
                {
                    using (var dbContext = new EcmsEntities())
                    {
                        ...
                        #region // Build:
                        dbContext.Invoices.DeleteObject(delInvoice);
						dbContext.SaveChanges();
            ...
            try
            {
					...
					using (var tranScope = new TransactionScope())
					{
						using (var db = new EcmsEntities())
						{
							...
							db.CustomerBalanceHistories.AddObject(customerBalanceHistory);
							db.CustomerBalanceHistories.AddObject(customerBalanceHistoryMaster);
			...
			try
			{
				using (var tranScope = new TransactionScope())
				{
					...
					if (invoiceReturn.Status == InvoiceStatus.Confirm && invoiceReturn.BusinessCode ==Const_BusinessCode.Business_201)
					{
						...
						dbContext.CustomerBalanceHistories.AddObject(customerBalanceHistoryMaster);
					}
