...
namespace Ecms.Biz.Class
{
    public class StockBiz
    {
        ...
        public List<Inv_StockInOutModel> StockInOutGet(
            string fromDate
            , string toDate
            , string stockInOutNo
            , string stockType
            , string stockCusName // customer name
                ...
                using (var dbContext = new EcmsEntities())
                {
                    var stockInOuts = from sio in dbContext.Inv_StockInOut.AsQueryable()
                                      ...
                                      select new Inv_StockInOutModel()
                                      {
                                          ...
                                          CustomerName = (sio.CustomerId ==null)? sio.CusName: c.CustomerName,
                    ...
                    {
                        Int32 type = Convert.ToInt32(stockType);
                        stockInOuts = stockInOuts.Where(x => x.Type == type);
                    }
                    ...
                    if (!string.IsNullOrEmpty(stockCusName))
                    {
                        stockInOuts = stockInOuts.Where(x => x.CustomerCode.Contains(stockCusName) || x.CustomerName.Contains(stockCusName));
                    }
        ...
        public Inv_StockInOut StockInOutCreate(Inv_StockInOut stockInOut, ref string alParamsOutError)
        {
            ...
            try
            {
				...
				if (stockInOut.Type == StockType.StockIn && stockInOut.status == StockStatus.StockConfirmed)
				{
					foreach (var item in stockInOut.Inv_StockInOutDetail)
					{
						...
						if (stockBalance == null)
						{
							...
							dbContext.Inv_StockBalance.AddObject(newStockBalance);
						}
						else
						{
							// update
                ...
                dbContext.Inv_StockInOut.AddObject(stockInOut);
		...
		public Inv_StockInOut StockInOutCreate(EcmsEntities context, Inv_StockInOut stockInOut, ref string alParamsOutError)
		{
			...
			try
			{
				...
				context.Inv_StockInOut.AddObject(stockInOut);
        ...
        public void GoodsCreate(List<Inv_Goods> listGoods, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var dbContext = new EcmsEntities())
                {
                    ...
                    {
                        dbContext.Inv_Goods.AddObject(item);
                    }
                    dbContext.SaveChanges();
