...
namespace Ecms.Biz
{
    public class ProductBiz : IProductBiz
    {
        ...
        public bool CategoryDelete(string categoryId, ref string alParamsOutError)
        {
            ...
            try
            {
                using (var _db = new EcmsEntities())
                {
                    ...
                    _db.Categories.DeleteObject(cate);
                    _db.SaveChanges();
                    #endregion
				...
				using (var _db = new EcmsEntities())
				{
					var result = from p in _db.Products
								 ...
								 select new ProductModel()
								 {
                                     ...
                                     DeadOfDay=p.DeadOfDay,
                                     FeeShip=p.FeeShip??0,
                                     ShipComment=p.ShipComment,
		...
		public bool ProductDelete(string productId, ref string alParamsOutError)
		{
			...
			try
			{
				using (var _db = new EcmsEntities())
				{
					...
					_db.Products.DeleteObject(product);
					_db.SaveChanges();
					#endregion
					return true;
				}
		...
		public Product ProductCreate(Product product, ref string alParamsOutError)
		{
			...
			try
			{
				using (var _db = new EcmsEntities())
				{
					_db.Products.AddObject(product);
					_db.SaveChanges();
				}
				return product;
			}
		...
		public Product ProductUpdate(Product product, ref string alParamsOutError)
		{
			...
			try
			{
				using (var _db = new EcmsEntities())
				{
                    ...
                    pro.RetailPrice = product.RetailPrice;
                    pro.ShipComment = product.ShipComment;
                    pro.FeeShip = product.FeeShip;
                    pro.DeadOfDay = product.DeadOfDay;
                    pro.Payment = product.Payment;
        ...
        /// <summary>
        /// update view count for user view product
        /// </summary>
        /// <param name="strProductId"></param>
        /// <param name="alParamsOutError"></param>
        /// <returns></returns>
        public bool ProductViewCountUpdate(string strProductId, ref string alParamsOutError)
        {
            #region // Temp
            string strFunctionName = "ProductViewCountUpdate";
            ArrayList alParamsCoupleError = new ArrayList(new object[]{
			            "strFunctionName", strFunctionName
            ...
            try
            {
                using (var _db = new EcmsEntities())
                {
                    Int32 productId = Convert.ToInt32(strProductId);
                    Product pro = _db.Products.Where(p => p.ProductId == productId).SingleOrDefault();

                    // Increatement
                    pro.ViewCount = (pro.ViewCount??0) +1;
    ...
    public class ProductModel
    {
        ...
        public String DeadOfDay { get; set; }
