...
namespace Ecms.Website.DBServices
{
    public class ProductService : BaseService
    {
        ...
        #region // ProductBestSaleUpdate
        public bool ProductViewCountUpdate(string strProductId, Page page)
        {
            try
            {
                string alParamsOutError = "";
                var result = this._productBiz.ProductViewCountUpdate(strProductId, ref alParamsOutError);
                if (!string.IsNullOrEmpty(alParamsOutError))
                {
                    throw GenServiceException(alParamsOutError);
                }
