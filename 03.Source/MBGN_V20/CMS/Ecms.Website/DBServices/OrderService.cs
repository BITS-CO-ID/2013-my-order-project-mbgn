    ...
    public class OrderService : BaseService
    {
            ...
            )
        {
            try
            {
                ...
                var returnCustomer = this._orderBiz.OrderDelete(
                                        orderId
                                        , ref alParamsOutError);
		...
		public bool RevertFirstOrderStatus01(string orderId, Page page)
		{
			try
			{
				string alParamsOutError = "";
				var result = _orderBiz.RevertFirstOrderStatus01(orderId, ref alParamsOutError);
		...
		public bool RevertOrderPendingByDetailCode(List<int> lstDetailId, Page page)
		{
			try
			{
				string alParamsOutError = "";
				var result = _orderBiz.RevertOrderPendingByDetailCode(lstDetailId, ref alParamsOutError);
		...
		public OrderDetail OrderDetailUpdate(OrderDetail orderDetail, Page page)
        {
            try
            {
                string alParamsOutError = "";
                var orderDetailReturn = _orderBiz.OrderDetailUpdate(orderDetail, ref alParamsOutError);
                if (!string.IsNullOrEmpty(alParamsOutError))
                    throw GenServiceException(alParamsOutError);
        ...
        public bool OrderDetailDelete(string orderDetailID, Page page)
        {
            try
            {
                string alParamsOutError = "";
                var orderDetailReturn = _orderBiz.OrderDetailDelete(orderDetailID, ref alParamsOutError);
                if (!string.IsNullOrEmpty(alParamsOutError))
                    throw GenServiceException(alParamsOutError);
                return orderDetailReturn;
